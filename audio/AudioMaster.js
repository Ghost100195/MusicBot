...
const { hasPermission, setOnBlacklist } = require('./permission/Permission.js');
    ...
    */
    ...
    _skip(userID, callback){ 
        ...
        if(!this.skippers.includes(userID)){
            ...
            callback({info: `Du hast schon für einen Skip gevotet! Benötigt: ${threshold - this.skippers.length} Skip-Wünsche zum überspringen!`});
        }
        ...
        if(this.skippers.length >= threshold){
            ...
            callback({info: `Dein Skip wird berücksichtig! Benötigt: ${threshold - this.skippers.length} Skip-Wünsche zum überspringen!`});
        }
    }
    ...
    _startMusic(queue, voiceChannel){
        if(!this.player.isActiv() && this.voiceChannel === null){
            ...
            this.voiceChannel.join().then((connection) => {
                this.reciver = new Reciver(connection);
